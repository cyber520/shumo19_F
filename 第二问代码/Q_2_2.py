...
def graw(result_):
    ...
    ax.set_ylabel('Y')
    ax.set_xlabel('X')
    plt.savefig('2_2.png',fig_size = (12,8))
    plt.show()
...
def greed_solution(data_ori, data, length_ori,b_ax,vec_a_b, alpha1, alpha2, beta1, beta2, theta, sigma, K=5):
    ...
    theta_old = 0
    data_ = data.loc[(data['Z坐标（单位: m）'] <= 8000) & (data['Z坐标（单位: m）'] >= 3000) & (data['投影值'] > 0)]
    data = data_.copy()
    for i in range(20):
        # print(data)
        ...
        print('第{0}次的cur.proj:{1}'.format(i, cur.values[-2]))
        ...
        # if count_ver == 1:
        #     min_val = min_val2
        #     # print('已连续进行1次垂直矫正，下次默认开始水平矫正')
        #     count_ver = 0
        # elif count_hor == 1:
        #     min_val = min_val1
        #     # print('已连续进行1次水平矫正，下次默认开始垂直矫正')
        #     count_hor = 0
        # else:
