...
def graw(result_):
    ...
    ax.set_ylabel('Y')
    ax.set_xlabel('X')
    plt.savefig('3_1.png',fig_size = (12,8))
    plt.show()


def greed_solution(data_ori, data, length_ori,b_ax,alpha1, alpha2, beta1, beta2, theta, sigma, K=2):
    ...
    for i in range(20):
        ...
        Ver_pos_tmp = data[data["校正点类型"] == 1]
        Hor_pos_tmp = data[data["校正点类型"] == 0]
        Ok_Ver_pos_tmp = data[(data["校正点类型"] == 1) & data['第三问点标记']==0]  # 获取所有垂直误差校验的DataFrame
        Prob_Ver_pos_tmp = data[(data["校正点类型"] == 1) & (data['第三问点标记'] == 1)]  # 获取所有垂直误差校验的DataFrame
        Ok_Hor_pos_tmp = data[(data["校正点类型"] == 0) & (data['第三问点标记']==0)]  # 获取所有水平误差校验的DataFrame
        Prob_Hor_pos_tmp = data[(data['校正点类型']==0) & (data['第三问点标记'] == 1)]
        ...
        if min_val in res_list1_candi_K:  # 垂直校验
            ...
            if final_step <= (theta - error_vertical - 1e-4) / 0.001 and final_step <= (
                    theta - error_horizontal - 1e-4) / 0.001:
            ...
            # drop_list.extend(drop_index_new)  # 当小于等于的加入到删除队列
